...
export default function ProfileLegalSection({
  ...
  return (
    <div className="pt-4 border-t border-brand-slate-100 dark:border-brand-slate-800">
      <button
        onClick={onToggle}
        className="flex items-center justify-between w-full p-4 rounded-xl glass-pill hover:border-brand-blue-500 dark:hover:border-brand-sky-300 transition-all text-xs font-mono font-bold text-brand-slate-600 dark:text-brand-slate-200 uppercase cursor-pointer"
      >
      ...
      {legalOpen && (
        <div className="p-4 glass-card rounded-b-xl text-left space-y-3">
          <p className="text-xs text-brand-slate-500 dark:text-brand-slate-300 italic">
            * {t.profile_legal_disclaimer}
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[11px] text-brand-slate-500 dark:text-brand-slate-200 font-mono">
            <div>
              <span className="block text-brand-slate-500 dark:text-brand-slate-350 text-[10px] uppercase font-bold">
